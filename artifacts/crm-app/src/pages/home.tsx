...
import { BookOpen, ChevronRight, Clock, Moon, Star } from "lucide-react";
      ...
      <div className="container mx-auto px-4 py-16 space-y-24">
            ...
            >
              ...
              <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-amber-300 text-xs font-semibold uppercase tracking-widest mb-1">{h.specialCollection}</p>
                    <h3 className="text-xl md:text-2xl font-serif font-bold text-white">{h.ramadanMode}</h3>
                    <p className="text-white/60 text-sm mt-0.5">{h.ramadanSub}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 bg-amber-400/20 border border-amber-400/30 rounded-xl px-5 py-2.5 text-amber-200 font-medium text-sm flex-shrink-0 hover:bg-amber-400/30 transition-colors">
                  {h.enterRamadan} <ChevronRight className="h-4 w-4 ms-1" />
                </div>
              </div>
            </motion.div>
          </Link>
        ...
        <section>
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-3xl font-serif font-bold text-foreground">{h.featuredReading}</h2>
            <Link href="/library">
              <Button variant="ghost" className="text-muted-foreground hover:text-primary">
                {h.viewAll} <ChevronRight className="ms-2 h-4 w-4" />
              </Button>
            </Link>
          </div>
                  ...
                  <div className="space-y-6">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="flex items-center text-sm text-muted-foreground">
                        ...
                        {featuredStory.readingTimeMinutes} {h.minRead}
                      </span>
                    </div>
                    <div>
                    ...
                    <div className="flex items-center gap-4 pt-4">
                      <div className="flex items-center text-secondary font-medium">
                        <Star className="me-2 h-5 w-5 fill-current" />
                      ...
                      <span className="text-primary font-medium flex items-center group-hover:underline underline-offset-4">
                        {h.beginReading} <ChevronRight className="ms-1 h-4 w-4" />
                      </span>
                    </div>
                  </div>
        ...
        <section>
          <div className="text-center mb-12">
            <h2 className="text-3xl font-serif font-bold text-foreground mb-4">{h.hallsOfKnowledge}</h2>
            <p className="text-muted-foreground max-w-2xl mx-auto">{h.hallsSub}</p>
          </div>
                    ...
                    <Card className="h-full hover:border-primary/50 transition-colors hover:shadow-md cursor-pointer group">
                      <CardContent className="p-6 text-center h-full flex flex-col justify-center items-center gap-4">
                        ...
                        <div>
                          ...
                          {category.nameAr && <p className="font-serif text-sm text-muted-foreground mb-2" dir="rtl">{category.nameAr}</p>}
                          <p className="text-sm text-muted-foreground">{category.storyCount} {h.stories}</p>
                        </div>
                      </CardContent>
                    </Card>
                  </motion.div>
          ...
          <section className="bg-primary text-primary-foreground rounded-2xl p-8 lg:p-12 relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-start">
              <div className="space-y-2">
                <h2 className="text-3xl font-serif font-bold">{h.community}</h2>
                <p className="text-primary-foreground/80 max-w-md">{h.communitySub}</p>
              </div>
              <div className="grid grid-cols-2 md:flex gap-8 md:gap-12">
                <div className="text-center">
                  ...
                  <div className="text-sm text-primary-foreground/80 uppercase tracking-wider font-medium">{h.seekers}</div>
                </div>
                <div className="text-center">
                  <div className="text-4xl font-bold font-serif mb-1">{summary.totalStories}</div>
                  <div className="text-sm text-primary-foreground/80 uppercase tracking-wider font-medium">{h.stories}</div>
                </div>
                <div className="text-center col-span-2 md:col-span-1">
                  <div className="text-4xl font-bold font-serif mb-1 text-secondary">{summary.storiesReadToday}</div>
                  <div className="text-sm text-primary-foreground/80 uppercase tracking-wider font-medium">{h.readToday}</div>
                </div>
              </div>
            </div>
          </section>
